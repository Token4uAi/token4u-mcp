...
async function main() {
  ...
  try {
    ...
    console.log(`✅ GLM: paid=${r.paidUsd} ct=${r.usage?.completionTokens} content=${r.content.length}chars reasoning=${r.reasoningContent?.length ?? 0}chars (${((Date.now()-t0)/1000).toFixed(1)}s)`);
  } catch (e: any) {
    console.log('❌', (e.body || e.message || '').slice(0, 300));
  }
}
main();
